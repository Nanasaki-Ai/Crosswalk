...
class MAN(nn.Module):
    def __init__(self, block=Bottleneck, layers=[3, 4, 6, 3], class_num=2, dropout=0.5):
        ...
        self.lateral_p1 = nn.Conv3d(8, 8*2, kernel_size=(5, 1, 1), stride=(2, 1 ,1), bias=False, padding=(2, 0, 0))
        self.lateral_res2 = nn.Conv3d(32,32*2, kernel_size=(5, 1, 1), stride=(2, 1 ,1), bias=False, padding=(2, 0, 0))
        self.lateral_res3 = nn.Conv3d(64,64*2, kernel_size=(5, 1, 1), stride=(2, 1 ,1), bias=False, padding=(2, 0, 0))
        self.lateral_res4 = nn.Conv3d(128,128*2, kernel_size=(5, 1, 1), stride=(2, 1 ,1), bias=False, padding=(2, 0, 0))
    ...
    def SlowPath(self, input_slow, lateral):
        x = self.slow_conv1(input_slow)
        x = self.slow_bn1(x)
        x = self.slow_relu(x)
    ...
    def FastPath(self, input_fast):
        lateral = []
        x = self.fast_conv1(input_fast)
        x = self.fast_bn1(x)
        x = self.fast_relu(x)
    ...
    def forward(self, input_slow, input_fast):
    
        fast, lateral = self.FastPath(input_fast[:, :, ::1, :, :])           
        slow = self.SlowPath(input_slow[:, :, ::2, :, :], lateral)     
...
def resnet50(**kwargs):
    ...
    """
    model = MAN(Bottleneck, [3, 4, 6, 3], **kwargs)
    return model
...
def resnet101(**kwargs):
    ...
    """
    model = MAN(Bottleneck, [3, 4, 23, 3], **kwargs)
    return model
...
def resnet152(**kwargs):
    """Constructs a ResNet-101 model.
    """
    model = MAN(Bottleneck, [3, 8, 36, 3], **kwargs)
    return model
...
def resnet200(**kwargs):
    """Constructs a ResNet-101 model.
    """
    model = MAN(Bottleneck, [3, 24, 36, 3], **kwargs)
    return model